...
import { RuleScenarioHarness } from '../harness/RuleScenarioHarness.js';
// ──────────────────────────────────────────────────────────────────────────────
// ScenarioBuilder
// ──────────────────────────────────────────────────────────────────────────────
/**
...
export class ScenarioBuilder {
     ...
     */
    expectStopLossAt(price, tolerance = 0.000001) {
     ...
     */
    expectTakeProfitAt(price, tolerance = 0.000001) {
     ...
     */
    expectActionExecuted(actionType) {
        ...
        for (const step of this.#steps) {
            switch (step.kind) {
                ...
                case 'expectStopLossAt': {
                    ...
                    assert.ok(Math.abs(sl - step.price) <= (step.tolerance ?? 0.000001), `[scenario: ${this.#description}] expectStopLossAt: expected SL ≈ ${step.price}, got ${sl}`);
                    break;
                }
                case 'expectTakeProfitAt': {
                    const tp = harness.currentTP;
                    assert.ok(tp !== undefined, `[scenario: ${this.#description}] expectTakeProfitAt(${step.price}): TP is undefined`);
                    assert.ok(Math.abs(tp - step.price) <= (step.tolerance ?? 0.000001), `[scenario: ${this.#description}] expectTakeProfitAt: expected TP ≈ ${step.price}, got ${tp}`);
                    break;
                }
                case 'expectActionExecuted': {
                    const executed = harness.executedActions.some(a => a.actionRef === step.actionType);
                    assert.ok(executed, `[scenario: ${this.#description}] expectActionExecuted(${step.actionType}): action not found in ${JSON.stringify(harness.executedActions.map(a => a.actionRef))}`);
                    break;
                }
            }
        }
